...
% last modified on 6/12/2024 by PD
...
%PowerV = abs(fft(Vd)).^2/L; 
%PowerW = abs(fft(Wd)).^2/L; 
...
% legend('Streamwise','Transverse','Vertical','k^-^5^/^3')
set(gca,'FontSize',25);
...
%Ef0V=mean(EnergyV(2:30));
%Ef0W=mean(EnergyW(2:30));
IntegralLengthScaleU=(Ef0U*UbarU)/(4*uprimeU.^2)
%IntegralLengthScaleV=(Ef0V*UbarV)/(4*uprimeV.^2);
%IntegralLengthScaleW=(Ef0W*UbarW)/(4*uprimeW.^2);
